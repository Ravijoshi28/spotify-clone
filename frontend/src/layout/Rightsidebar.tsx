...
function Rightsidebar() {
    ...
    const {users,fetchUsers,onlineUser,userActivities}=useChatUser()
    ...
    <div className="h-full bg-zinc-900 rounded-lg flex flex-col" >
        <div className="p-4 flex justify-between items-center border-b border-zinc-800">
            <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold">What are they listening?</h2>
            </div>
        </div>
                    ...
                    className="cursor-pointer hover:bg-zinc-800/50 p-3 rounded-md  transition-colors group">
                        <div className="flex items-start gap-3">
                            <div className="relative">
                                <Avatar className="size-10 border-zince-800">
                                    <AvatarImage 
                                    src={u.imageUrl}
                                    alt={u.fullName}/>
                                ...
                                ${onlineUser.has(u.clerkId)? "bg-green-500":"bg-zinc-500"} `} aria-hidden="true"></div>
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                    <span className="font-medium text-sm text-white">{u.fullName}</span>
                                    {isplaying && <Music2 className="size-3.5 text-emerald-400 shrink-0"/>}
                                </div>
                                {isplaying?(
